...
fn main(@builtin(global_invocation_id) global: vec3<u32>) {
    ...
    // TODO: Everything is seems right but for some reason,
    if x + param.kernel_width > param.source_width || y + param.kernel_height > param.source_height {
        output[(param.source_width * y) + x] = f32(x);
        return;
    }
    ...
    for (var trow = 0u; trow < param.kernel_height; trow = trow + 1u) {
        let source_row = (y + trow) * param.source_width + x; // offset for 2d-to-1d vec
